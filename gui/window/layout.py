...
class WindowLayoutMixin:
    def _build_ui_legacy(self) -> None:
        ...
        self.preview_canvas.sticker_selected.connect(self.on_sticker_selection_changed)
        self.preview_canvas.sticker_deleted.connect(self.on_delete_sticker_clicked)
    ...
    def _build_ui_compact(self) -> None:
        ...
        self._video_preview_widget.sticker_dragged.connect(self.on_preview_sticker_dragged)
        self._video_preview_widget.sticker_scaled.connect(self.on_preview_sticker_scaled)
        self._video_preview_widget.sticker_selected.connect(self.on_sticker_selection_changed)
        self._video_preview_widget.sticker_deleted.connect(self.on_delete_sticker_clicked)
        self._video_preview_widget.watermark_scale_changed.connect(self.on_watermark_scale_dragged)
        ...
        self.sticker_search_input = QLineEdit()
        self.sticker_search_input.setPlaceholderText("🔍 Nhập tên để tìm nhanh sticker...")
        self.sticker_search_input.textChanged.connect(self.on_sticker_search_changed)
        ...
        self.sticker_start_time = QTimeEdit()
        self.sticker_start_time.setDisplayFormat("HH:mm:ss")
        self.sticker_start_time.setToolTip("Thời gian bắt đầu xuất hiện sticker (HH:mm:ss)")
        self.sticker_start_time.timeChanged.connect(self.on_basic_settings_changed)
        
        self.sticker_end_time = QTimeEdit()
        self.sticker_end_time.setDisplayFormat("HH:mm:ss")
        self.sticker_end_time.setToolTip("Thời gian ẩn sticker (HH:mm:ss, 00:00:00 = hết video)")
        self.sticker_end_time.timeChanged.connect(self.on_basic_settings_changed)
        ...
        font_grid.addWidget(self.font_group_combo, 0, 1, 1, 4)
        font_grid.addWidget(self._field_label("Font"), 1, 0)
        font_grid.addWidget(self.font_combo, 1, 1, 1, 4)
        font_grid.addWidget(self._field_label("Cỡ chữ"), 2, 0)
        font_grid.addWidget(self.font_size_spin, 2, 1)
        font_grid.addWidget(self.font_size_slider, 2, 2, 1, 2)
        font_grid.addWidget(self.font_size_value, 2, 4)
        ...
        font_grid.addWidget(self.text_effect_combo, 3, 3, 1, 2)
        ...
        font_grid.addWidget(self._field_label("Tìm sticker"), 5, 0)
        font_grid.addWidget(self.sticker_search_input, 5, 1, 1, 4)
        font_grid.addWidget(self._field_label("Loại Sticker"), 6, 0)
        font_grid.addWidget(self.sticker_combo, 6, 1, 1, 4)
        font_grid.addWidget(self._field_label("Bắt đầu"), 7, 0)
        font_grid.addWidget(self.sticker_start_time, 7, 1, 1, 2)
        font_grid.addWidget(self._field_label("Kết thúc"), 7, 3)
        font_grid.addWidget(self.sticker_end_time, 7, 4)
        font_grid.addWidget(self._field_label("Tối đa từ/dòng"), 8, 0)
        font_grid.addWidget(self.max_words_spin, 8, 1)
        font_grid.addWidget(self.font_color_btn, 8, 2, 1, 3)
        font_grid.addWidget(self._field_label("Palette màu chữ"), 9, 0)
        font_grid.addLayout(self._build_palette_row("fontColor", FONT_COLOR_SWATCHES), 9, 1, 1, 4)
        font_grid.addWidget(self._field_label("Palette màu viền"), 10, 0)
        font_grid.addLayout(self._build_palette_row("strokeColor", STROKE_COLOR_SWATCHES), 10, 1, 1, 4)
    ...
    def _switch_page(self, index: int) -> None:
        ...
        if index == 1:
            ...
            if vp is not None:
                ...
                if source_path:
                    vp.load_video(source_path, auto_play=False)
