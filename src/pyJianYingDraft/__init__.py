...
# Import jianying_controller only on Windows
ISWIN = (sys.platform == 'win32')
if ISWIN:
    try:
        from .jianying_controller import JianyingController, ExportResolution, ExportFramerate
    except ImportError:
        # Set to False if Windows dependencies are missing
        ISWIN = False
...
# Backward compatibility for enum classes - using proxy classes
class _DeprecatedEnum:
    """Enum proxy class with deprecation warning"""
...
# Define backward compatibility classes for jianying_controller only on Windows
if ISWIN:
...
# Base __all__ list (common for all platforms)
__all__ = [
    "FontType",
    "MaskType",
    "FilterType",
    ...
    # Backward compatible snake_case classes
    "Script_file",
    "Draft_folder",
    "Shrink_mode",
    "Extend_mode",
...
# Add jianying_controller related exports only on Windows
if ISWIN:
    __all__.extend([
        "JianyingController",
        "ExportResolution",
