...
def main():
    if len(sys.argv) > 1:
        # Install quiet excepthook immediately for any CLI mode to suppress
        # PyInstaller's "Unhandled exception in script" popup modal on Windows
        root = ROOT_DIR
        if root not in sys.path:
            sys.path.insert(0, root)
        try:
            from tools.dub_studio_pipeline import setup_quiet_excepthook
